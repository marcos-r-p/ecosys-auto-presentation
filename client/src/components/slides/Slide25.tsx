      ...
      <div className="max-w-7xl w-full flex flex-col gap-6 md:gap-10 lg:gap-14">
        ...
        >
          <h2 className="text-3xl font-bold text-white mb-2">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-red-400 to-red-600 drop-shadow-lg">: Impacto, Roadmap e Integração</span>
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            {impacts.map((impact, index) => (
              <motion.div
                ...
                className="relative bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-5 hover:border-white/30 hover:bg-white/10 transition-all"
              >
                {/* Icon */}
                ...
                {/* Title */}
                <h4 className="text-base font-bold text-white text-center mb-3 leading-tight">{impact.title}</h4>
              ...
              >
                ...
                <div
                  className={`relative bg-white/5 backdrop-blur-md border ${
                    phase.highlight ? "border-[#2B7FFF]/50" : "border-white/10"
                  } rounded-xl p-4 h-full hover:border-white/30 hover:bg-white/10 transition-all`}
                >
                  {/* Phase Title */}
                  <div className="flex items-center gap-2 mb-3">
                    <Calendar className={`w-5 h-5 ${phase.highlight ? "" : "text-blue-400"}`} style={phase.highlight ? { color: '#2B7FFF' } : {}} />
                    <h4 className={`text-base font-bold leading-tight ${phase.highlight ? "" : "text-white"}`} style={phase.highlight ? { color: '#2B7FFF' } : {}}>
                      {phase.phase}
                    </h4>
                  </div>
                  ...
                  {/* Deliverable */}
                  <div className={`mt-3 pt-3 border-t ${phase.highlight ? "border-[#2B7FFF]/30" : "border-white/10"}`}>
                    <p className="text-xs font-bold text-white/60 mb-1">Entregável:</p>
                    <p className={`text-sm font-bold leading-tight ${phase.highlight ? "" : "text-white"}`} style={phase.highlight ? { color: '#2B7FFF' } : {}}>
                      {phase.deliverable}
                    </p>
                  </div>
                </div>
